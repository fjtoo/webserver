...
public:
    ...
    static const int FILENAME_LEN = 200;       // 常量 文件名的最大长度

    // 以下枚举类型供HTTP请求解析的有限状态机使用，见游双书P136
    // 分主从两个状态机，主状态机负责以行为单位解析HTTP报文，从状态机负责解析行内部的信息
...
private:
    ...
    int m_check_index;  // 当前正在解析的字符在读缓冲区位置
